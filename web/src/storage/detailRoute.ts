 ...
 *   - Matches the project's existing convention (ADR-011 backend bearer tokens are base58).
 *   - Alphabet is URL-safe with no `-`, `_`, or `=` padding noise.
 *   - Visually unambiguous: omits 0/O/I/l.
 *
 *   - 与项目已有约定一致 (ADR-011 后端 bearer token 即为 base58).
 ...
 *   control. Real authorization still goes through ADR-011 bearer tokens.
 *
 *   该编码按设计可逆 — 持有 token 的任何人都能还原 (source_key, video_id).
 *   这是 URL 表现层的清理, 不是访问控制. 真正的鉴权仍走 ADR-011 bearer token.
 *
 * Key exports / 主要导出:
...
// The encoder rejects 0x1F in either field, so decoding can split the payload unambiguously.
//
// fieldSeparator 在编码负载内分隔 source_key 与 video_id.
// 编码器会拒绝任一字段中的 0x1F, 因此解码时可以无歧义地分割负载.
const fieldSeparator = "\x1F";

// Bitcoin/IPFS base58 alphabet — same one used by the Go backend (ADR-011).
//
// Bitcoin/IPFS base58 字母表 — 与 Go 后端 (ADR-011) 一致.

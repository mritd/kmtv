 ...
 *
 * Responsibilities / 职责:
 *   - Provide create, update, and remove mutations.
 *   - Invalidate the ["admin", "users"] cache after every successful mutation
 *
 *   - 提供 create, update 和 remove mutations.
...
export function useUsersMutations() {
  ...
  return {
    /**
     * create adds a user account and invalidates the users list.
     *
     * create 新建用户账户并使用户列表缓存失效.
     */
    create: useMutation({
    ...
    /**
     * remove deletes a user account by id.
     *
     * remove 通过 id 删除用户账户.
     */
    remove: useMutation({

 ...
 *
 * Responsibilities / 职责:
 *   - Provide create, update, remove, and sync mutations.
 *   - Invalidate the ["admin", "subscriptions"] cache after every successful mutation
 *
 *   - 提供 create, update, remove 和 sync mutations.
...
export function useSubscriptionsMutations() {
  ...
  return {
    /**
     * create adds a subscription. The server immediately syncs its sources, so both lists refresh.
     *
     * create 新建订阅. 服务端会立即同步其中的视频源, 因此订阅和源列表都会刷新.
     */
    create: useMutation({
    ...
    /**
     * update updates an existing subscription by id.
     *
     * update 通过 id 更新已有订阅.
     */
    update: useMutation({

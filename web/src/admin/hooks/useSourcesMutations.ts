...
export function useSourcesMutations() {
  ...
  return {
    /**
     * create creates a new source and invalidates the sources list.
     *
     * create 新建视频源并使源列表缓存失效.
     */
    create: useMutation({
    ...
    /**
     * update updates an existing source by id.
     *
     * update 通过 id 更新已有视频源.
     */
    update: useMutation({
    ...
    /**
     * bulkSetEnabled enables or disables multiple sources by id array.
     *
     * bulkSetEnabled 通过 id 数组批量启用或禁用视频源.
     */
    ...
    /**
     * importBundle imports the JSON bundle submitted by SourceImportForm.
     *
     * importBundle 导入 SourceImportForm 提交的 JSON bundle.
     */
    ...
    /**
     * checkAll triggers health probes for all sources.
     *
     * checkAll 触发所有视频源的健康探测.
     */

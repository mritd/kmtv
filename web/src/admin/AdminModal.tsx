...
function AdminModalBody({ payload, onClose }: { payload: AdminModalPayload; onClose: () => void }): ReactNode {
  // Bind mutation hooks before the switch so every render follows the same hook order
  // and each modal branch can invoke its operation without prop drilling.
  //
  // 在 switch 前绑定 mutation hook, 保证每次渲染的 hook 顺序一致,
  // 同时让各弹窗分支无需透传即可调用对应操作.
...
//
// assertNever 是 TypeScript 穷尽性守卫.

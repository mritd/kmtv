 ...
 *
 * Responsibilities / 职责:
 *   - Require username in both modes and password only in create mode.
 *   - Require matching password confirmation in create mode with live mismatch feedback.
 *   - Hide password fields in edit mode; ChangePasswordForm handles password updates.
 *   - Dispatch create or update based on whether a user is provided.
 *   - Show a toast when the mutation fails.
 *
 *   - 两种模式都要求 username, 仅新建模式要求 password.
 *   - 新建模式要求二次确认密码并实时提示不一致.
 *   - 编辑模式隐藏密码字段, 密码修改由 ChangePasswordForm 处理.
 *   - 根据是否传入 user 分发 create 或 update mutation.

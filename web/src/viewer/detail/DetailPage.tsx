 ...
 *
 * Route token / 路由令牌:
 *   The :token path parameter is an opaque base58 encoding of (source_key, video_id)
 *   (see storage/detailRoute.ts, ADR-013). DetailPage decodes it on every render and
 *   shows a "link no longer available" status when the token is malformed.
 *
 *   :token 路径参数是 (source_key, video_id) 的 base58 不透明编码
 *   (详见 storage/detailRoute.ts, ADR-013). DetailPage 在每次渲染时解码,
 ...
 *   - 路由路径 /detail/:token 不得更改 (token 编码方案 ADR-013)
 */
import {
  useCallback,
  useEffect,
 ...
 *
 * Route param: token — opaque base58 encoding of (source_key, video_id),
 ...
 *
 * 路由参数: token — (source_key, video_id) 的 base58 不透明编码,
  ...
  return (
    <main className="page detail-page">
      <StatusState
        ...
        description={t("detail.loadFailedHelp")}
        tone="error"
      />
...
function DetailPageContent({
  ...
  const [bundleState, setBundleState] = useState(() =>
    initialBundleState(source, id, location.state),
  );
  ...
  const pendingEpisodeSelection = useRef<{
    sourceKey: string;
    videoID: string;
    episodeIndex: number;
  } | null>(null);
  ...
  const setBundle = (
    updater: SourceBundle | ((current: SourceBundle) => SourceBundle),
  ) => {
  ...
  const currentSource =
    selectedSource(bundle, selectedSourceID) ??
    bundle.sources[0] ??
    singleSource(source, id);
  ...
  const [state, baseDispatch] = useReducer(playbackReducer, undefined, () => {
    ...
    return (
      detailStore.getState().entries[key]?.playback ??
      createInitialPlaybackState()
    );
  });
  ...
  const canRenderRecoverableDetailError =
    detail.isError && bundle.sources.length > 1;
  ...
  const backgroundSourceSignature = bundle.sources
    ...
    .filter((source) => {
      const status =
        bundle.details[sourceKeyID(source.source_key, source.video_id)]?.status;
  ...
  useEffect(() => {
    ...
    if (
      ...
      previous.id === id &&
      previous.locationState === location.state
    ) {
  ...
  useEffect(() => {
    ...
    setBundle((current) => {
      const next = upsertSourceBundleDetail(
        current,
        currentSourceKey,
        currentVideoID,
        detail.data,
  ...
  useEffect(() => {
    ...
    setBundle((current) => {
      if (
        current.details[sourceKeyID(currentSourceKey, currentVideoID)]
          ?.status === "ready"
      ) {
        return current;
      }
      const next = markSourceBundleDetailFailed(
        current,
        currentSourceKey,
        currentVideoID,
  ...
  useEffect(() => {
    if (
      hasResolvedBundle ||
      !currentDetail?.title ||
      recoveryAttemptedRoute.current === currentRouteID
    ) {
      return;
    }
    ...
    void api
      ...
      .then((response) => {
        ...
        const match = (response.results ?? []).find((result) =>
          matchesDetail(result, detailForSource),
        );
        if (!match) {
          setBundleState((current) => {
            if (
        ...
        const recovered = upsertSourceBundleDetail(
          bundleWithCurrentSource(
            bundleFromSearchResult(match),
            sourceKey,
            videoID,
          ),
          sourceKey,
  ...
  useEffect(() => {
    ...
    const candidates = bundle.sources.filter((source) => {
      ...
      return (
        ...
        status !== "ready" &&
        status !== "failed" &&
        !backgroundLoadingIDs.current.has(`${generation}:${id}`)
      );
    });
      ...
      void api
        ...
        .then((nextDetail) => {
          setBundle((current) => {
            ...
            if (
              bundleSourceIdentity(current) !== expectedBundleID ||
              !bundleContainsSource(current, candidate)
            ) {
              return current;
            }
  ...
  useEffect(() => {
    ...
    if (
      pendingSelection?.sourceKey === currentSourceKey &&
      pendingSelection.videoID === currentVideoID
    ) {
      const detailStatus =
        bundle.details[sourceKeyID(currentSourceKey, currentVideoID)]?.status;
      if (detailStatus !== "ready") {
        return;
      }
      const pendingGroupIndex = firstPlayableGroup(groups);
      const pendingGroup = groups[pendingGroupIndex] ?? [];
      const pendingEpisodeIndex =
        pendingSelection.episodeIndex < pendingGroup.length
          ? pendingSelection.episodeIndex
          : 0;
  ...
  function selectSourceID(sourceIDValue: string) {
    ...
    const episodeIndex =
      desiredEpisodeIndex < group.length ? desiredEpisodeIndex : 0;
    const episode = group[episodeIndex];
    const detailStatus =
      bundle.details[sourceKeyID(nextSource.source_key, nextSource.video_id)]
        ?.status;
    pendingEpisodeSelection.current =
      !episode ||
      (desiredEpisodeIndex >= group.length && detailStatus !== "ready")
        ? {
            sourceKey: nextSource.source_key,
            videoID: nextSource.video_id,
            episodeIndex: desiredEpisodeIndex,
          }
        : null;
      ...
      <section className="detail-player-grid">
        <div className="detail-main">
          ...
          <section className="detail-copy">
            <p className="muted">
              {[displayDetail.type, displayDetail.year, displayDetail.area]
                .filter(Boolean)
                .join(" | ")}
            </p>
        ...
        <aside className="detail-sidebar">
          ...
          <EpisodePicker
            ...
            onSelect={(episodeIndex, episode) =>
              resolveEpisode(state.groupIndex, episodeIndex, episode)
            }
          />
          {state.status === "failed" ? (
...
function initialBundleState(
  ...
  return {
    bundle: singleSourceBundle(sourceKey, videoId),
...
function selectedSource(
  ...
  return (
    bundle.sources.find((source) => sourceID(source) === selectedID) ?? null
  );
}
...
function groupsForSource(
  ...
  const detail =
    bundle.details[sourceKeyID(source.source_key, source.video_id)]?.detail;
...
function sourcePickerItems(
  ...
  return bundle.sources.map((source) => {
    ...
    const status =
      id === currentSourceID && isCurrentLoading
        ? "loading"
        : (detail?.status ?? "idle");
    return {
      key: id,
...
function bundleContainsSource(
  bundle: SourceBundle,
  source: SourceResult,
): boolean {
  const id = sourceKeyID(source.source_key, source.video_id);
  return bundle.sources.some(
    (candidate) => sourceKeyID(candidate.source_key, candidate.video_id) === id,
  );
}
...
function bundleWithCurrentSource(
  bundle: SourceBundle,
  sourceKey: string,
  videoId: string,
): SourceBundle {
  if (
    bundle.sources.some(
      (source) => sourceID(source) === sourceKeyID(sourceKey, videoId),
    )
  ) {
    return bundle;
  }
  return {
    ...bundle,
    sources: [singleSource(sourceKey, videoId), ...bundle.sources],
  };
}
...
function matchesDetail(
  ...
  if (
    ...
    result.title.trim().toLocaleLowerCase() !==
      detail.title.trim().toLocaleLowerCase()
  ) {
    return false;
  }
...
function isSearchResult(value: unknown): value is SearchResult {
  ...
  return (
    ...
    value.sources.every(isSourceResult)
  );
}
...
function isSourceResult(value: unknown): value is SourceResult {
  if (
    ...
    return (
      ...
      (Array.isArray(value.episodes) && value.episodes.every(isEpisode))
    );
  }
  return false;
}
...
// handles short episodes, while the ratio threshold handles long videos with inaccurate durations.
//
// playbackCompleted 将最后 30 秒或最后 5% 视为播放完成. 时间阈值覆盖短剧集,
// 比例阈值覆盖 duration 不够精确的长视频.
...
/**
 * resumePositionFor returns a positive checkpoint only when its source, media, group, and episode
 * all match the episode currently being loaded.
 *
 * resumePositionFor 仅在来源, 媒体, 分组和集数都匹配当前加载集时返回正数检查点.
 *
 * Synthetic bundle identities, missing checkpoints, different episodes, and zero positions return
 * undefined so the player starts normally instead of seeking to unrelated progress.
 *
 * 合成 bundle 标识, 缺失检查点, 不同集数和零位置都返回 undefined,
 * 使播放器正常开始, 而不是跳转到无关进度.
 */
function resumePositionFor(
  sourceKey: string,
  videoID: string,
  groupIndex: number,
  episodeIndex: number,

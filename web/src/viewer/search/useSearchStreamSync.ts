        ...
        .searchStream(
          ...
          (event) => {
            ...
            if (event.type === "progress") {
              ...
              // Coerce only a missing or null result array to empty. Preserve every valid server card
              // exactly as received; title spacing, punctuation, and separate variants are not normalized
              // or merged in this client bridge.
              //
              // 仅将缺失或 null 的结果数组转换为空数组. 每个有效服务端卡片都按原样保留;
              // 此客户端桥接不会归一化或合并标题空格, 标点及独立变体.

 ...
 *
 * ADR refs: ADR-011 (base58 bearer tokens, locked auth model)
 *
...
// ProbeResult stores the probe verdict separately from the token snapshot. Each render derives
// one status from those two inputs, so snapshot updates and probe completion cannot race through
// competing setStatus calls.
//
// ProbeResult 将探测结论与 token 快照分开保存. 每次渲染都从这两个输入派生唯一 status,
// 因此快照更新和探测完成不会通过多个 setStatus 调用互相竞争.
type ProbeResult =
  | { kind: "pending" }
  | { kind: "anonymous"; user: User }
  | { kind: "unauthenticated" };

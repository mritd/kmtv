 ...
 *
 * Detail path change (ADR-013) / 详情页路径变更 (ADR-013):
 ...
 *   base58 encoding of (source_key, video_id) — see storage/detailRoute.ts.
 *   Legacy /detail/:source/:id URLs are intentionally NOT redirected; old shared
 *   links fall through to the wildcard route and redirect to "/".
 *
 *   详情路由原为 /detail/:source/:id, 会在 URL 中暴露第三方源域名. 现改为 /detail/:token,
 *   token 为 (source_key, video_id) 的 base58 不透明编码 — 详见 storage/detailRoute.ts.
 *   旧 /detail/:source/:id 链接不做单独兼容, 而是落入通配路由并重定向到 "/".
 *
 * Key exports / 主要导出:
 *   AppRoutes
 *

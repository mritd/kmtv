...
const (
	...
	maxWatchHistoryClearFutureLead = time.Second
)
...
func (s *Store) ClearWatchHistory(userID, clearedAtMS int64) error {
	...
	// Accept a clear timestamp up to one second ahead of server time. This keeps
	// the next timestamp generated by the same client ordered after the clear.
	// Clamp a larger future value to server time so a bad device clock cannot
	// make legitimate checkpoints stale until the server clock catches up.
	//
	// 允许清空时间最多比服务端快 1 秒, 保证同一客户端接着生成的时间戳仍晚于本次清空.
	// 更大的未来时间会被钳制到服务端当前时间, 避免错误设备时钟让正常 checkpoint
	// 在服务端时钟追上之前持续被判定为 stale.
	if now := time.Now().UnixMilli(); clearedAtMS > now+maxWatchHistoryClearFutureLead.Milliseconds() {
		clearedAtMS = now
	}
	tx, err := s.db.Begin()
	if err != nil {

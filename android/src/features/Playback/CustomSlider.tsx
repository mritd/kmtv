...
export function CustomSlider({ value, onDragStart, onDragEnd, testID, _panForTest }: CustomSliderProps) {
  ...
  const thumbX = Math.max(0, width * clamp01(display) - thumbSize / 2);
  ...
  return (
    ...
    >
      ...
      <View style={[styles.trackFill, { width: Math.max(0, width * clamp01(display)) }]} />
      <View style={[styles.thumb, { width: thumbSize, height: thumbSize, left: thumbX, top: -(thumbSize - TRACK_HEIGHT) / 2 }]} />
    </View>
  );
}

const styles = StyleSheet.create({
  root: { height: 32, justifyContent: "center" },
  trackBg: { height: TRACK_HEIGHT, borderRadius: TRACK_HEIGHT / 2, backgroundColor: "rgba(255,255,255,0.3)" },
